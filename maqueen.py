...
class Maqueen:
    ...
    Add some stuff by @javacasm
    License: GNU
    """
    def __init__(self):
        self.rgbleds = neopixel.NeoPixel(microbit.pin15, 4)
        if self.is_maqueen():
            print("maqueen initializado")
        else: 
            print('No se ha detectado maqueen')

    def is_maqueen(self):
        i2c_devs = microbit.i2c.scan()
        return len(i2c_devs) > 0 and 16 in i2c_devs
        
    def set_led_PWM(self, lednumber, value):
        """
        0 - 255 front LEDS
        0 - left LED (P8)
        1 - right LED (P12)
        """
        if lednumber == 0:
            microbit.pin8.write_analog(value)
        elif lednumber == 1:
            microbit.pin12.write_analog(value)        
    ...
    def read_distance(self,echo,trigger):
        """
        ...
        divider = 42
        maxtime = 250 * divider
        echo.read_digital()  # just for setting PULL_DOWN on echo
        ...
        duration = machine.time_pulse_us(echo, 1, maxtime)
        distance = duration//divider
        return distance


    def read_distance_back(self):
        return self.read_distance(microbit.pin0,microbit.pin1)
    
    def read_distance_front(self):
        return self.read_distance(microbit.pin2,microbit.pin1)
    ...
    def set_motor(self, motor, value):
        ...
        try:
            ...
            print('Error accediendo a maqueen',oe)
